...
if [ "$#" -lt 1 ]; then
    vmajor=$(grep "VITROIO_DHT_DEMO_VERSION_MAJOR   " ../common/global_consts.h)
    MAJOR=("${vmajor: -1}")
    vminor=$(grep "VITROIO_DHT_DEMO_VERSION_MINOR   " ../common/global_consts.h)
    MINOR=("${vminor: -1}")
    vpatch=$(grep "VITROIO_DHT_DEMO_VERSION_PATCH   " ../common/global_consts.h)
    PATCH=("${vpatch: -1}")
    vrc=$(grep "VITROIO_DHT_DEMO_VERSION_RC   " ../common/global_consts.h)
    RC=("${vrc: -1}")
    vid=$(grep "VITRIOIO_DHT_DEMO_FIRMWARE_ID   " ../common/global_consts.h)
    FIRMWARE_ID=("${vid: -1}")
...
cd $WORK_DIR
echo -e "\nBuilding SWU image...\n"
PRODUCT_NAME="vitroio_dht_demo_nodes_upgrade_package"
